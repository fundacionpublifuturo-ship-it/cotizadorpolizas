...
PLANES_ZURICH = [
    {"nombre": "Plan Vida Zurich", "selector_valor": "vida_zurich",
     "elegible": lambda c: c.tipo_seguro == "vida"},
    {"nombre": "Plan Hogar Zurich", "selector_valor": "hogar_zurich",
     "elegible": lambda c: c.tipo_seguro == "hogar"},
]
...
async def cotizar_zurich(cliente, headless=True):
    return await _cotizar_generico("Zurich", "https://agentes.zurich.com.co/login",
                                    'input[name="usuario"]', 'input[name="clave"]',
                                    "ZURICH_USER", "ZURICH_PASS", PLANES_ZURICH, cliente, headless)
...
async def cotizar_equidad(cliente, headless=True):
    return await _cotizar_generico("Equidad Seguros", "https://portal.equidadseguros.coop/login",
                                    'input[name="usuario"]', 'input[name="clave"]',
                                    "EQUIDAD_USER", "EQUIDAD_PASS", PLANES_EQUIDAD, cliente, headless)


async def cotizar_axa_colpatria(cliente, headless=True):
    return await _cotizar_generico("AXA Colpatria", "https://agentes.axacolpatria.co/login",
                                    'input[name="usuario"]', 'input[name="clave"]',
...
async def cotizar_todas(cliente: DatosCliente, headless: bool = True) -> list[dict]:
    tareas = [
        ...
        cotizar_zurich(cliente, headless),
